...
print_rules_home_style() {
    ...
    for ((i=0; i<${#rule_nodes[@]}; i++)); do
        ...
        echo -e "  ${GREEN}[$((i+1))]${NC} ${CYAN}${rule_nodes[$i]}${NC} → ${CYAN}${rule_targets[$i]}${NC} (监听: ${YELLOW}${rule_ports[$i]}${NC})${wl_info}"
    done
    echo ""
}
...
do_delete() {
    ...
    while true; do
        ...
        local del_info="${del_node} → ${rule_targets[$del_idx]} (监听: ${rule_ports[$del_idx]})"
